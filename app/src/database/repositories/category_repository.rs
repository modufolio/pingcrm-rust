...
impl CategoryRepository {
    ...
    pub async fn delete(&self, category_id: i32) -> Result<usize, diesel::result::Error> {
        use crate::database::schema::categories::dsl::*;
    ...
    pub async fn paginate(
        &self,
        params: &QueryParams,
    ) -> Result<PaginatedResult<Category>, diesel::result::Error> {
        use crate::database::schema::categories::dsl::*;
    ...
    pub async fn find_with_params(
        ...
        let mut count_query = categories.filter(account_id.eq(acct_id)).into_boxed();
        if let Some(ref pattern) = search_pattern {
            count_query = count_query.filter(name.like(pattern.clone()));
        }
        let total = count_query.count().get_result::<i64>(&mut conn).await?;

        let mut query = categories.filter(account_id.eq(acct_id)).into_boxed();
        if let Some(ref pattern) = search_pattern {
            query = query.filter(name.like(pattern.clone()));
        }
        query = Self::apply_category_sort(query, &params.sort);
        let offset = (params.page.number - 1) * params.page.size;
        query = query.limit(params.page.size).offset(offset);

        let items = query.load::<Category>(&mut conn).await?;
    ...
    fn apply_category_sort(
        query: categories::BoxedQuery<'static, diesel::sqlite::Sqlite>,
        sorts: &[(String, SortDirection)],
    ) -> categories::BoxedQuery<'static, diesel::sqlite::Sqlite> {
        use crate::database::schema::categories::dsl::*;
...
impl From<DbPool> for CategoryRepository {
...
impl crate::database::JsonApiRepository<Category> for CategoryRepository {
    async fn find_by_id(&self, id: i32) -> Result<Option<Category>, diesel::result::Error> {
    ...
    async fn create(&self, new_item: NewCategory) -> Result<Category, diesel::result::Error> {
    ...
    async fn load_by_foreign_key_in(
        ...
        match foreign_key {
            "account_id" => {
                categories::table
                    .filter(categories::account_id.eq_any(&ids))
                    .load::<Category>(&mut conn)
                    .await
            }
            "parent_id" => {
                categories::table
                    .filter(categories::parent_id.eq_any(&ids))
                    .load::<Category>(&mut conn)
                    .await
            }
            _ => Ok(vec![]),
